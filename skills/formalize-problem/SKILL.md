...
Pin down every dimension of the system/trust model. If any dimension is left vague, the investigation will produce ambiguous results. Consult `references/model.md` for the domain-appropriate checklist of dimensions that must be specified. Every field must have a concrete answer, not "TBD".
...
Not all questions are equally worth investigating. Prioritize by consequence — ask: "If we resolved this question, who would care and why?" Consult `references/model.md` for domain-specific examples of how to rank by importance.
...
For each hypothesis, check whether it contradicts a known impossibility or lower bound. Consult `references/impossibility-results.md` for the domain-relevant impossibility results and lower bounds.
...
## Model Assumptions
[One field per dimension from references/model.md. Every dimension must have a concrete answer.]
